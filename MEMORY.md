...
**Maintenance mode + Jama Connect MCP™ pivot operational.** All four implementation phases (0, 1, 2, 4.5) closed. **Jama Software announced general availability of Jama Connect MCP™ on 2026-05-04** — their official, branded, paid add-on MCP server for cloud and self-hosted Jama Connect deployments. This makes operational the planned pivot from the 2026-05-02 strategic decision: our public `jama-mcp-server` repo's role inverts from "foundation we extend" to "reference architecture / proof we engaged with the protocol seriously before Jama released theirs." No further phases of our MCP server will be built; Jama Connect MCP is the supply-side from now on.

**Software Engineer use case packaged as consulting-proposal artifacts** (saved gitignored under `docs/internal/`): IG2200 demo project configuration spec, and Software Engineer Workflow Brief. Both reference Jama Connect MCP™ as the supply-side; our existing repo is referenced as proof-of-execution credibility. Earlier strategic framing in `docs/internal/jama-poc-strategic-positioning.md`.

**Next session intent (per user):** elementary capability test against the Sandbox tenant. Primary objective: probe Jama Connect MCP's tool surface directly (does `pm2.jamacloud.com` already have it enabled? what tools does it expose? does it support `create_relationship` / structural reads?). Secondary objective: comparison baseline with our existing tools (Phases 1 + 4.5) via Claude Code chat. Parallel test in GitHub Copilot. **No new code expected.** Result feeds final consulting-proposal positioning before Preston is engaged with a request for Jama Connect MCP access.
...
| 2026-05-04 | **Strategic pivot from build-first to consulting-proposal-first for the Software Engineer use case.** Triggered by recognition that (a) Preston's three-line user story does not warrant the weeks of build effort that a full prototype + IG2200 reconfiguration would require; (b) packaging the design as a consulting proposal protects against asymmetric exploitation of unpaid work; (c) artifacts double as engagement-kickoff documents if the proposal is accepted. **Two artifacts produced this session, both gitignored under `docs/internal/`:** (i) `specs/ig2200-project-configuration-spec.md` — full IG2200 v2 demo project structure, ~60 items, 5 software modules (PC/OD/AM/DL/BM), Source Code URL custom field on Software Requirement type to preserve B-vs-C trace-link path optionality; (ii) `briefs/software-engineer-use-case-workflow-brief.md` — six-frame schematic SVG storyboard of the developer's IDE workflow, including an explicit human-in-the-loop confirmation gate (Frame 4) per Preston's preamble principle. Tier 2 VS Code Extension target identified (command palette + QuickPick + editor insertion + status bar; no sidebar/webview/activity-bar branding). Trace-link Path A (native Jama relationship to a "Software Component" item type) ruled out after IG2200 sandbox structure inspection — no such item type exists in stock medical-device Jama project structures; B (URL custom field) and C (external link via `POST /items/{id}/links`) preserved as discovery-phase decision in any consulting engagement | Avoids exploitation risk; calibrates artifact effort to input quality (schematic SVG mockups, not photo-realistic screenshots — match the three-line input); positions the existing MCP Server foundation (Phases 1+4.5, public on GitHub with passing CI) as proof-of-execution credibility asset rather than infrastructure-in-progress. System Engineer use case received verbatim from Preston (system-spec compliance-officer AI agent) but explicitly out of scope for this brief; companion brief deferred to a separate engagement deliverable. Next-session intent: capability-test how much of the SE use case is fulfillable today with existing Phase 1 + 4.5 tools via Claude Code chat (and parallel test in GitHub Copilot) before any further build commitment |
| 2026-05-04 | **Jama Software announced general availability of `Jama Connect MCP™`.** Official, branded, paid add-on MCP server for both cloud and self-hosted Jama Connect deployments. Procurement is mediated by the customer's Jama Customer Success Manager. Per the Jama Connect Help directory: "allows you to connect approved MCP-compatible clients, such as Integrative Development Environment (IDE) or Command-Line Interface (CLI) tools, to Jama Connect to securely retrieve information and execute supported actions through controlled tool calls." Both reads and writes; specific tool surface and pricing not yet known. **Operational consequences:** (a) the planned pivot from the 2026-05-02 strategic decision is now actionable rather than predictive; (b) our public `jama-mcp-server` repo's role formally inverts to "reference architecture / proof we engaged with the protocol seriously before Jama released theirs" — Phases 5 and 6 of our MCP server are definitively cancelled; (c) the consulting-proposal artifacts (spec + brief in `docs/internal/`) updated to reference Jama Connect MCP as the supply-side foundation, with our repo cited as proof-of-execution credibility; (d) Phase 4.5 `create_comment` becomes deprecation candidate once Jama Connect MCP's write surface is confirmed to include comment creation. **Next session pivot in test scope:** primary objective is now to probe Jama Connect MCP's tool surface against `pm2.jamacloud.com` directly (does the Sandbox tenant already have it enabled? what tools are exposed?), with our existing tools as comparison baseline rather than primary subject | Today's announcement makes operational the strategic decision made 2026-05-02; "head-start" investment in Phases 1 and 4.5 paid off as designed by giving us working knowledge of the Jama REST surface, OAuth flow, item/relationship/comment data shapes, and gotchas — none of which is wasted, all of which transfers directly to integrating against the official MCP. The user will engage Preston to request Jama Connect MCP access for the Sandbox tenant after the artifacts are cleaned up |
