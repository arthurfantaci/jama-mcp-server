...
| 2026-05-01 | Phase 3 brainstorm + plan-write combined into one session (vs Phase 2's hygiene → plan → execution three-session split); plan committed at [`docs/superpowers/plans/2026-05-01-jama-mcp-server-phase-3-minikube.md`](docs/superpowers/plans/2026-05-01-jama-mcp-server-phase-3-minikube.md); issue #8 open; branch `feat/phase-3-minikube` pushed | No new spec; decisions live in the plan + these rows. Phase 3 is the FINAL phase, so condensing the prep saves one session-handoff cycle without sacrificing controller-context discipline (execution still gets a fresh session). Validates the six-section handoff prompt structure for the second time |
| 2026-05-01 | Phase 3 deeper readiness probe: `_readyz` module-level handler verifies OAuth token freshness via new public `JamaClient.is_token_fresh()` (in-memory check, no I/O). Liveness stays at `/health`. Lifespan now eagerly calls `JamaClient.warm_token_cache()` at startup so readiness reports `ready` the moment startup completes | Splitting probes lets a transiently-stale-token Pod drop from rotation without flapping into restart loops. Eager warm surfaces credential errors at startup rather than 30 seconds after first user request. KG anticipated the split in the Phase 2 `FastMCP custom_route module-level handler pattern` entity |
| 2026-05-01 | Phase 3 NetworkPolicy: same-namespace `podSelector` ingress on TCP/8765, egress allow-list of kube-dns + 443/TCP to anywhere (minus standard private/link-local CIDRs). No Jamacloud IP pinning | Jama Cloud CDN ranges are not published as stable CIDRs; pinning would be brittle. README quickstart documents `minikube start --cni=calico` because the default `auto` CNI accepts NetworkPolicy resources but does NOT enforce them |
| 2026-05-01 | Phase 3 manifest CI: `.github/workflows/k8s-validate.yml` runs kubeconform 0.6.7 schema validation against `kustomize build` output, scoped via path filters to `k8s/**` + workflow file. SHA256 pinning of the kubeconform tarball deferred to a Phase 3.5+ hardening pass | Mirrors Phase 2's `docker-build.yml` philosophy of running ONLY when relevant files change. Schema-only is the right strictness for a low-churn demo project transitioning to maintenance; kube-linter and full kind-cluster apply are overkill for the spec's scope. Brings CI check count from 6 to 7 |
