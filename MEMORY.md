...
| 4.5 | `create_comment` write tool — narrow Phase 4.5 carve-out for compliance-agent runtime | In progress (Issue #9, branch `feat/create_comment`) |
...
**Phase 4.5 — `create_comment` write tool *(in progress, Issue #9, branch `feat/create_comment`)*:**
...
- **`JamaClient.create_comment(item_id, project_id, body)`** posts to `POST /rest/latest/comments` with the canonical Jama nested shape. Defaults: `commentType="GENERAL"`, `inReplyTo=0`.
- **`@server.tool() create_comment(ctx, item_id, project_id, body)`** in `tools.py` — same shape as read tools; HITL checkpoint is the orchestrating skill's responsibility, not the server's.
- **Tests** — 86 unit/protocol tests passing (Phase 1+2's 84 + 2 new for `create_comment` client + tool). Live integration test gated on `JAMA_INTEGRATION_COMMENT_ITEM_ID` + `JAMA_INTEGRATION_COMMENT_PROJECT_ID`; leaves a timestamped self-identifying comment behind.
...
| 2026-05-02 | Phase 4.5 narrow carve-out: add `create_comment` write tool to this PoC (Issue #9, branch `feat/create_comment`). Single MCP write tool wrapping `POST /rest/latest/comments`; supports both demo personas at runtime (Persona 1 records requirement→code linkage as a comment; Persona 2 posts compliance findings as comments). Authoritative request schema obtained from the user during the implementation session: `body` is nested `{"text": "..."}`, `location` is nested `{"item": <id>, "project": <id>}` (project required), `commentType="GENERAL"`, `inReplyTo=0` for top-level. Phase 4.5 conventions codified in CLAUDE.md (`_request` json_body extension, nested body/location handling, HITL-as-caller-responsibility for write tools, env-gated live integration test pattern) | Comments are additive-only (no delete, no modify after write) so the AI-safety blast radius is much smaller than `create_item`/`update_item`/`delete_item`. This special-case carve-out does NOT reverse the broader 2026-05-02 strategic decision to defer Phase 5's full write surface to the official Jama MCP Server. If the official MCP also exposes comment creation when access lands, this tool can be deprecated in favor of composition |
