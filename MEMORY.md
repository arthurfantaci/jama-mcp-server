...
| 2026-05-01 | Phase 3 (Kubernetes / Minikube deployment) descoped; project enters maintenance mode at end of Phase 2; engineering record preserved at git tag `archive/phase-3-attempt-2026-05-01`; Issue #8 closed with descoping comment; documentation updated on main in commit `2daa6e7` to remove Phase 3 references from spec, README, MEMORY.md, CLAUDE.md, setup.md, and the GitHub issue template (plan files retained as historical artifacts) | FastMCP's user `lifespan=` parameter does not run at HTTP server startup in streamable-HTTP mode (only per-MCP-session via the low-level Server class), invalidating the original `/readyz` design that depended on a module-level state container being populated at Pod start. Architectural mitigation attempts (move state to `main_http` via `anyio.run`; serve foreground + warm background via `anyio.create_task_group`) intersected with a Calico CNI startup race that intermittently blocked egress on Pod start, plus an Apple Virtualization.framework watchdog timeout kernel panic on macOS 26.5 (Tahoe) under sustained Docker Desktop + Minikube load. Decision: ship the working Phases 0–2 deliverable rather than continue Phase 3 hardening. KG entities `Phase 3 K8s Deployment Attempt (Jama MCP Server, descoped)`, `FastMCP HTTP Lifespan Gotcha`, and `macOS Tahoe Docker Desktop Hypervisor Watchdog Panic` preserve the engineering insights for future sessions and cross-project work |
