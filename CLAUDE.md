...
- **Fixed UID/GID 1001 (`jama` user)** in Dockerfile RUN. Pinned for non-root container execution; numeric UID ≥1000 satisfies orchestrator security policies that require non-system user identities. The `useradd warning: jama's uid 1001 is greater than SYS_UID_MAX 999` in the build log is expected and intentional.
...
- **`_parse_envelope` returns the full envelope (no `data`-key requirement) when `return_envelope=True`**, and only enforces the `data` key when the caller asks for unwrapped data. Some Jama write endpoints return a `meta`-only response with the new resource's `id` and `location` and no `data` field; these endpoints must use `return_envelope=True`.
- **POST `/rest/latest/comments` request body uses Jama's nested shape**: `body` is `{"text": "..."}` (NOT a flat string), `location` is `{"item": <id>, "project": <id>}` (project is required, NOT derivable server-side). Defaults: `commentType="GENERAL"`, `inReplyTo` **omitted entirely** for top-level comments — sending `inReplyTo: 0` triggers a server-side `NullPointerException` in Jamacloud's parent-comment lookup (verified against `pm2.jamacloud.com` 2026-05-02; Jama returns HTTP 500 with `parentComment is null`). Surface `project_id` as a required tool parameter rather than auto-fetching it via `get_item` — agents reach `create_comment` having already done the requirement lookup, so the project ID is in their context.
- **POST `/rest/latest/comments` response is `meta`-only** (`{"meta": {"id": <new_id>, "location": ..., ...}}`); there is no `data` field carrying the full comment. `JamaClient.create_comment` therefore synthesises the returned `Comment` from the new ID plus the inputs the caller already provided; timestamp and author fields are left `None`. Callers that need a fully-populated comment can issue a follow-up GET against `/rest/latest/comments/{id}`.
...
- **Jama exposes no `DELETE /comments/{id}` in the REST API** (verified against `pm2.jamacloud.com/api-docs/` 2026-05-02). Integration-test artifacts and demo-runtime comments must be cleaned via the Jama UI. This is consistent with Jama's audit-trail design philosophy (comments are append-only by intent, supporting regulated-industry traceability requirements). Mitigations for write-test artifact accumulation: (a) use a designated "scratch" item that never appears in demo flows; (b) rely on timestamps + run-identifying body text for visual filtering during manual cleanup; (c) accept accumulation — comments are cheap and the audit-trail framing is itself a demo positive.
- **`commentType` enum values** (verified against the Jama Swagger schema 2026-05-02): `GENERAL`, `QUESTION`, `PROPOSED_CHANGE`, `ACCEPTED_COMMENT`, `REJECTED_COMMENT`, `ISSUE`, `DECISION`, `DECISION_REQUEST`. Jama validates server-side and rejects unknown values with HTTP 400. The MCP tool exposes `comment_type` as an optional parameter (default `"GENERAL"`) so agentic callers can surface semantic intent — notably, Persona 2 (compliance reviewer) should emit `"ISSUE"` for non-compliant requirement findings rather than `"GENERAL"`, which renders with the appropriate issue affordance in the Jama UI.
