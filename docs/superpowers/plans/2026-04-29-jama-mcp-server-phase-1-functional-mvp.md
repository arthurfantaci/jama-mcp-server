...
Note: `host` and `port` are constructor arguments on the installed `FastMCP` class
(verified via `inspect.signature(FastMCP.__init__)`). `FastMCP.run()` only accepts
`transport` and `mount_path`. This corrects an earlier plan revision which
incorrectly placed `host`/`port` on `run()`. The lazy-import noqa was also tightened
from `F401, PLC0415` to `PLC0415` only — `F401` does not apply because `tools` is
used immediately on the next line.
...
Note: `Context` is imported at runtime, not under `TYPE_CHECKING`. FastMCP's
tool registration calls `typing.get_type_hints()` on each registered closure
to discover the context parameter. With `from __future__ import annotations`
active, all annotations are strings; `get_type_hints` resolves them at
runtime against the enclosing module's globals. A `TYPE_CHECKING`-only
import would raise `NameError` at registration time.

`_Context = Context[Any, Any, Any]` is a fully-parameterised alias so the
function signatures satisfy mypy strict's `[type-arg]` rule without needing
runtime parameterisation. `JamaClient` and `FastMCP`, by contrast, only
appear in annotations and stay under `TYPE_CHECKING`.
